...
public class ExitView extends View {
	...
	public ExitView(SuperBabyMainActivity context) {
		super(context);
	...
	private void DrawButton(Canvas canvas) {
		int X = (int) (superbabyActivity.screen_width / 2 - 70* superbabyActivity.height_mul);
		int Y = (int) (superbabyActivity.screen_height / 2);
        ...
        if(isclick[1]){
        	//startExitAnim();
			context.handler.sendEmptyMessage(superbabyActivity.WELCOME);
        }
		...
		canvas.drawText("back", X + 100* superbabyActivity.height_mul, Y, paint2);
	}
	...
	private void DrawTitle(Canvas canvas) {
		Paint paint = new Paint();
		paint.setAntiAlias(true);
		paint.setTextSize(50* superbabyActivity.height_mul);
		paint.setColor(Color.parseColor("#f97f09"));
		int y = (int) (superbabyActivity.screen_height / 12);
		canvas.drawText("Exit?", superbabyActivity.screen_width/2 - 40* superbabyActivity.height_mul, y+100* superbabyActivity.height_mul, paint);
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		if(event.getAction() == MotionEvent.ACTION_DOWN){
			...
			if(x > X-20 * superbabyActivity.height_mul&& x < X + 50 * superbabyActivity.height_mul&& y > Y - 40* superbabyActivity.height_mul && y < Y + 40* superbabyActivity.height_mul)
				isclick[0] = true;
			if(x > X+80 * superbabyActivity.height_mul&& x < X + 150 * superbabyActivity.height_mul&& y > Y - 40 * superbabyActivity.height_mul&& y < Y + 40* superbabyActivity.height_mul)
				isclick[1] = true;
		}
